...
export default {
    ...
    actions: {
        ...
        async QueryLiquidityPoolByPoolCoinDenom({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params: { ...key }, query = null }) {
            try {
                const queryClient = await initQueryClient(rootGetters);
                let value = (await queryClient.queryLiquidityPoolByPoolCoinDenom(key.pool_coin_denom)).data;
                commit('QUERY', { query: 'LiquidityPoolByPoolCoinDenom', key: { params: { ...key }, query }, value });
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryLiquidityPoolByPoolCoinDenom', payload: { options: { all }, params: { ...key }, query } });
                return getters['getLiquidityPoolByPoolCoinDenom']({ params: { ...key }, query }) ?? {};
            }
            catch (e) {
                throw new SpVuexError('QueryClient:QueryLiquidityPoolByPoolCoinDenom', 'API Node Unavailable. Could not perform query: ' + e.message);
            }
        },
        async QueryLiquidityPoolByReserveAcc({ commit, rootGetters, getters }, { options: { subscribe, all } = { subscribe: false, all: false }, params: { ...key }, query = null }) {
            try {
                const queryClient = await initQueryClient(rootGetters);
                let value = (await queryClient.queryLiquidityPoolByReserveAcc(key.reserve_acc)).data;
                commit('QUERY', { query: 'LiquidityPoolByReserveAcc', key: { params: { ...key }, query }, value });
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryLiquidityPoolByReserveAcc', payload: { options: { all }, params: { ...key }, query } });
                return getters['getLiquidityPoolByReserveAcc']({ params: { ...key }, query }) ?? {};
            }
            catch (e) {
                throw new SpVuexError('QueryClient:QueryLiquidityPoolByReserveAcc', 'API Node Unavailable. Could not perform query: ' + e.message);
            }
        },
